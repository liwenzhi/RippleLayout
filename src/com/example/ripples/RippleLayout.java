...
public class RippleLayout extends FrameLayout {
    ...
    private static final int DEFAULT_DURATION = 350;       //默认的显示涟漪效果的时间
    private static final int DEFAULT_FADE_DURATION = 75;    //涟漪褪去的时间
    private static final float DEFAULT_DIAMETER_DP = 10;     //直径多少dp
    private static final float DEFAULT_ALPHA = 0.2f;          //透明度，透明度为1表示完成显示
    private static final int DEFAULT_COLOR = 0xff43c0ec;        //涟漪是产生的效果颜色
    private static final int DEFAULT_BACKGROUND = Color.TRANSPARENT;  //背景颜色，跟backGround一样的
    private static final boolean DEFAULT_HOVER = true;                  //是否盘旋
    private static final boolean DEFAULT_DELAY_CLICK = true;            //是否延迟点击
    private static final boolean DEFAULT_PERSISTENT = false;             //涟漪后效果是否一直那样显示（这里不是循环显示涟漪，而是整个条目一直显示最后的效果）
    private static final boolean DEFAULT_SEARCH_ADAPTER = false;        //适配器？，这是适配器的Item才能使用这个属性，否则会报错
    private static final boolean DEFAULT_RIPPLE_OVERLAY = false;        //产生的波纹是否覆盖
    private static final int DEFAULT_ROUNDED_CORNERS = 0;               //是否显示圆角

    private static final int FADE_EXTRA_DELAY = 50;                    //消失延迟的时间
    private static final long HOVER_DURATION = 2500;                   //盘旋时间
    ...
    public RippleLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        rippleColor = a.getColor(R.styleable.RippleLayout_mrl_rippleColor, DEFAULT_COLOR);
        rippleDiameter = a.getDimensionPixelSize(
                R.styleable.RippleLayout_mrl_rippleDimension,
                (int) dpToPx(getResources(), DEFAULT_DIAMETER_DP)
        );
        rippleOverlay = a.getBoolean(R.styleable.RippleLayout_mrl_rippleOverlay, DEFAULT_RIPPLE_OVERLAY);
        rippleHover = a.getBoolean(R.styleable.RippleLayout_mrl_rippleHover, DEFAULT_HOVER);
        rippleDuration = a.getInt(R.styleable.RippleLayout_mrl_rippleDuration, DEFAULT_DURATION);
        rippleAlpha = (int) (255 * a.getFloat(R.styleable.RippleLayout_mrl_rippleAlpha, DEFAULT_ALPHA));
        rippleDelayClick = a.getBoolean(R.styleable.RippleLayout_mrl_rippleDelayClick, DEFAULT_DELAY_CLICK);
        rippleFadeDuration = a.getInteger(R.styleable.RippleLayout_mrl_rippleFadeDuration, DEFAULT_FADE_DURATION);
        rippleBackground = new ColorDrawable(a.getColor(R.styleable.RippleLayout_mrl_rippleBackground, DEFAULT_BACKGROUND));
        ripplePersistent = a.getBoolean(R.styleable.RippleLayout_mrl_ripplePersistent, DEFAULT_PERSISTENT);
        rippleInAdapter = a.getBoolean(R.styleable.RippleLayout_mrl_rippleInAdapter, DEFAULT_SEARCH_ADAPTER);
        rippleRoundedCorners = a.getDimensionPixelSize(R.styleable.RippleLayout_mrl_rippleRoundedCorners, DEFAULT_ROUNDED_CORNERS);
